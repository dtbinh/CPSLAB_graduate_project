...
pSlog = []; pTlog = []; problog=[];
NUM_SMP_INIT = 100;
NUM_SMP = NUM_SMP_INIT;
FailLog = zeros(50,4);
%% ---- [ Start target tracking ] -- %
for timestamps = 1:50%length(pseq)-H-2%30
    %tic;
   % the current position of the target
    ...
    %% CCDP
    %tic;
    [ uv_opt, uw_opt, predict_s ] = fcn_track_ccdp( pS, pEst, H, T, ccdp, threshold_track, costparam,nextstate );
    ...
    pT = pT_est;
    pT_smp = mvnrnd(pT_est,eye(2)*pEst(1).sig^2,NUM_SMP)'; % insert noise
   ...
   % problog = [problog,currprob];
    %toc;
     ...
     if(isinf(uv_opt)&&isinf(uw_opt))
         ...
         FailLog(timestamps,1) = NUM_SMP;
         break;
     end
    % within sensing region
    pT_rel = bsxfun(@minus, pT_smp,pS(1:2)); % relative position of sampled pT
    check_dist = sqrt(pT_rel(1,:).^2+pT_rel(2,:).^2);
    check_angle = [cos(pS(3)),sin(pS(3))]*pT_rel/check_dist;
    ...
    if(sum(gmap.gmap(sub2ind(size(gmap.gmap),check_collision(1),check_collision(2)))>0)~=0)
        ...
        successflag =0;
        FailLog(timestamps,4) = NUM_SMP;
        fprintf('Fail:collision\n');
        break;
    end
    
    Trackfail = check_dist > Rs | check_angle< cos(ThetaS/2); %index : out of sensing region
    Visfail = zeros(1,size(check_dist,2)); %index : occlusion
    for ii=1:size(check_dist,2)
        visibility_line = unique(round(([linspace(pT_smp(2,ii),pS(2),20)-gmap.ylim(1);linspace(pT_smp(1,ii),pS(1),20)-gmap.xlim(1)])/gmap.glen)'+1,'rows'); 
        Visfail(ii) = (sum(gmap.gmap(sub2ind(size(gmap.gmap),visibility_line(:,1),visibility_line(:,2)))>0)~=0) & (~Trackfail(ii));
    end
    
    FailLog(timestamps,2)=sum(Trackfail); % the number of cases where the target is out of the sensing region
    FailLog(timestamps,3)=sum(Visfail); % the number of occluded cases
    
    
    
    NUM_SMP = NUM_SMP-sum(FailLog(timestamps,:));
    if(NUM_SMP==0)
        break;
    end
