...
if(~exist('pseq','var'))
    ...
    mapnum = 1;
    sim_iternum = 1;
    load(sprintf('./target_traj/TargetTraj_Map%d_%d_2.mat',mapnum,sim_iternum)); % load a trajectory
end
if(~exist('H','var'))
     H=2; % horizon
end
...
for timestamps = 1:50%length(pseq)-H-2%30
   ...
   % the current position of the target
    ...
    %vs = [1,1]';
    vs_ang = 1.4;
    Useed=linspace(Urng(1),Urng(2),20);
    Vseed = linspace(Vrng(1),Vrng(2),10);
   % [ uv_opt,uw_opt] = fcn_track_greedy_objective( pS, pT, T, Rs, ThetaS,gmap ,Vseed, Useed, vp);
   % pS = [s;phi]+[cos(phi),0;sin(phi),0;0,1]*[uv_opt,uw_opt]'*T;
    [nextvel]=fcn_track_greedy(pS,pT,pEst,T,Rs, ThetaS,gmap,vs,vs_ang,vp);
    pS=pS+nextvel;
...
%     end
   ...
   if(0)
       ...
       % plot(pseq(1,timestamps+(1:H)),pseq(2,timestamps+(1:H)),'r','linewidth',2); hold on;
        if(timestamps>1)
            quiver(pTlog(1,end-1),pTlog(2,end-1),vp(1),vp(2),'m','markersize',10); hold on;
        end
